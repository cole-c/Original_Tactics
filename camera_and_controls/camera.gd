...
func _physics_process(delta: float) -> void:
	...
	if(ray_cast_3d.is_colliding()):
		...
		if(collider.is_in_group("Character") && Input.is_action_just_pressed("click_L")):
			game.click_character(collider.get_parent())
		if (collider.is_in_group("Tile")):
			var new_hovered_tile = collider.get_parent()
			game.hover_tile(new_hovered_tile)
			if(Input.is_action_just_pressed("click_L")):
				game.click_tile(new_hovered_tile)
	else: 
		game.hover_default()
